...
for data_file in os.listdir(curr_data_path):
    ...
    new_df.to_pickle(os.path.join(today_temp_path, '{}_learning.p'.format(data_file[:-4])))
    new_df.to_csv(os.path.join(today_temp_path, '{}_learning.csv'.format(data_file[:-4])), index=False)
