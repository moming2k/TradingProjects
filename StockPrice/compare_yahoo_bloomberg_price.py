...
def is_match(y_df, b_df):
    ...
    df = df[df.index > datetime.datetime(2006, 11, 20)]
    count = 0
...
split_list = np.array_split(stock_list[:100], process_num)


def process_list(split_stock_list):
    split_stock_list = list(split_stock_list)
    match_stock_list = []
    for yahoo_file in os.listdir(os.path.join(root_path, yahoo_price)):
        try:
            ...
            def transfer_str_to_datetime(s):
                if isinstance(s, datetime.datetime):
                    return s
            ...
            if not isinstance(bloomberg_df.index[1], datetime.datetime):
                bloomberg_df['Date'] = bloomberg_df.index
                bloomberg_df['Date'] = bloomberg_df['Date'].apply(transfer_str_to_datetime)
                bloomberg_df = bloomberg_df.set_index('Date')

            yahoo_df = yahoo_df[yahoo_df.Volume != 0]
            yahoo_df['Date'] = yahoo_df.index
            yahoo_df['Date'] = yahoo_df['Date'].apply(transfer_str_to_datetime)
            yahoo_df = yahoo_df.set_index('Date')

            if is_match(y_df=yahoo_df, b_df=bloomberg_df):
                with open('matched_list.txt', 'a') as f:
                    f.write(yahoo_file)
                    f.write('\n')
...
result = process_list(stock_list)
