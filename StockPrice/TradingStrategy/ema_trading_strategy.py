...
# File name: ema_trading_strategy
# Author: warn
# Date: warn
...
result_path = '{}EMASimulateResult'.format(datetime.datetime.today().strftime('%Y%m%d'))
...
def ema_trading_strategy(file_name):
    ...
    for period1 in range(14, 4, -2):
        for period2 in range(period1 * 2, 31, 2):
            for period3 in range(7, 21, 2):
                ...
                ema_s = abstract.EMA(stock_price, timeperiod=period1) if period1 != 1 else stock_price['close']
                ema_l = abstract.EMA(stock_price, timeperiod=period2)

                dma = ema_s - ema_l
                ama = abstract.EMA(dma.to_frame('close'), timeperiod=period3)
