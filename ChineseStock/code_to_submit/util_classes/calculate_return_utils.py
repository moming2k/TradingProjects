...
from average_portfolio import AveragePortfolio, AccountHedge399300
from util_function import load_stock_info


class CalculateReturnUtils(Constant):
    """ This class add run up info to data """

    def __init__(self, root_path, trading_list_path=None, stock_price_path=None):
        self.init_path(root_path)

        if trading_list_path is None:
            trading_list_path = self.TRADING_DAYS_20170228_PATH

        self._trading_days_list = pd.read_pickle(trading_list_path)
        self._stock_price_path = self.STOCK_DATA_PATH if stock_price_path is None else stock_price_path
    ...
    def calculate_return_and_wealth(self, info):
        ...
        try:
            ...
            wealth_series.to_csv(os.path.join(wealth_path, '{}_raw.csv'.format(file_name)))
            alpha_series.to_pickle(os.path.join(wealth_path, '{}_alpha.p'.format(file_name)))
            alpha_series.to_csv(os.path.join(wealth_path, '{}_alpha.csv'.format(file_name)))
        ...
        return wealth_series, alpha_series
    ...
    def generate_buy_only_return_df(self, return_path, holding_days, info_type=None, stoploss_rate=None,
        ...
        if report_path is None:
            report_path = self.INSIDER_EXE_GT2_PATH
    ...
    def calculate_portfolio_return(self, report_df, portfolio_num, transaction_cost=0.):
        portfolio = AveragePortfolio(total_num=portfolio_num, root_path=self.ROOT_PATH, total_value=self.initial_wealth,
                                     stock_price_path=self._stock_price_path,
                                     transaction_cost=transaction_cost, price_type=self.STOCK_CLOSE_PRICE)
        alpha_hedge_portfolio = AveragePortfolio(root_path=self.ROOT_PATH, total_num=portfolio_num,
                                                 total_value=self.initial_wealth,
                                                 stock_price_path=self._stock_price_path,
                                                 transaction_cost=transaction_cost, price_type=self.STOCK_CLOSE_PRICE,
    ...
    def calculate_trade_info(self, announce_date, ticker_info, stoploss_rate, holding_days,
        ...
        :param buy_price_type: use which price to buy this stock
        :param sell_price_type: Stock price to sell in data
        :param after_price_type: after the stock has been sell, use which price to sell
        :param stock_price_path: where to load stock price data
