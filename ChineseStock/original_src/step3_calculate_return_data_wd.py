    ...
    if sell_info.empty:
        for day in trading_days[holding_days:]:
            ...
            if not sell_info.empty:
                # here use open price
                sell_price = sell_info.loc[sell_info.first_valid_index(), const.STOCK_ADJPRCWD]
...
if __name__ == '__main__':
    ...
    result_df.to_pickle(os.path.join(today_path, 'wd_hday_{}.p'.format(holding_days)))
    result_df.to_csv(os.path.join(today_path, 'wd_hday_{}.csv'.format(holding_days)), encoding='utf8')
